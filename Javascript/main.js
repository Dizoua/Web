...
function confirmOrder() {
  ...
  localStorage.setItem("orders", JSON.stringify(orders)); // cập nhật LS để admin thấy
  // nếu bạn giảm stock trên products, nhớ cập nhật products LS:
...
// ===== SYNC WITH ADMIN VIA LOCALSTORAGE =====
  ...
  try {
    ...
    if (adminProducts.length > 0) {
      products = adminProducts.map((adminProd) => {
        const existingProd = products.find((p) => p.id === adminProd.id);
        if (existingProd) {
          // Cập nhật thông tin nhưng giữ các thuộc tính bổ sung
          return {
            ...existingProd,
            ...adminProd,
            img: adminProd.image || adminProd.img || existingProd.img,
          };
        }
        return {
          id: adminProd.id,
          ...
          desc: adminProd.desc || adminProd.description || "",
          features: adminProd.features || ["Chất lượng cao"],
          sizes: adminProd.sizes || ["38", "39", "40", "41", "42"],
          colors: adminProd.colors || ["Đen", "Trắng"],
          img:
            adminProd.image ||
            adminProd.img ||
            "https://via.placeholder.com/350x280/667eea/ffffff?text=Sneaker+Image",
    ...
    if (adminOrders.length > 0) {
      orders = adminOrders.map((adminOrder) => ({
        id: adminOrder.id,
        user: adminOrder.user || adminOrder.customer,
        details: adminOrder.details || adminOrder.products || [],
        payment: adminOrder.payment || "cod",
    ...
    // Đồng bộ đánh giá
    const adminReviews = JSON.parse(localStorage.getItem("reviews")) || [];
...
// Hàm lưu đơn hàng lên localStorage để admin thấy
function saveOrderToAdmin(order) {
  try {
    ...
    // Cập nhật stock sản phẩm trong admin
    const adminProducts = JSON.parse(localStorage.getItem("products")) || [];
    order.details.forEach((item) => {
      const adminProduct = adminProducts.find((p) => p.id === item.id);
      if (adminProduct && adminProduct.stock !== undefined) {
        adminProduct.stock = Math.max(0, adminProduct.stock - item.qty);
      }
    });
    localStorage.setItem("products", JSON.stringify(adminProducts));

    console.log("Đã lưu đơn hàng lên admin");
  } catch (error) {
    console.error("Lỗi lưu đơn hàng lên admin:", error);
  }
}
...
function saveUserToAdmin(user) {
  try {
    ...
    console.log("Đã lưu người dùng lên admin");
  } catch (error) {
    console.error("Lỗi lưu người dùng lên admin:", error);
  }
}

// Hàm lưu đánh giá lên localStorage
function saveReviewToAdmin(review) {
  try {
    const adminReviews = JSON.parse(localStorage.getItem("reviews")) || [];
    adminReviews.push(review);
    localStorage.setItem("reviews", JSON.stringify(adminReviews));
    console.log("Đã lưu đánh giá lên admin");
  } catch (error) {
    console.error("Lỗi lưu đánh giá lên admin:", error);
  }
}
...
function register() {
  ...
  users.push(newUser);
  saveUserToAdmin(newUser); // Lưu lên admin
...
function submitReview(productId) {
  ...
  reviews.push(newReview);
  saveReviewToAdmin(newReview); // Lưu lên admin
...
function confirmOrder() {
  ...
  orders.push(order);
  saveOrderToAdmin(order); // Lưu lên admin

  // Cập nhật stock trong local
  orderDetails.forEach((item) => {
    const product = products.find((p) => p.id === item.id);
    if (product) {
      product.stock -= item.qty;
...
function saveProfile() {
  ...
  saveUserToAdmin(currentUser); // Lưu lên admin
...
document.addEventListener("DOMContentLoaded", function () {
  // Đồng bộ dữ liệu từ admin trước
  syncWithAdmin();
  ...
  if (users.length === 0) {
    ...
    users.push(demoUser);
    saveUserToAdmin(demoUser); // Lưu user demo lên admin
  }
  ...
  window.addEventListener("storage", function (e) {
    if (["products", "orders", "users", "reviews"].includes(e.key)) {
      syncWithAdmin();
